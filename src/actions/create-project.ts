...
export async function createProject(idea: string, stage: StartupStage, _messages: unknown[] = []) {
    try {
        ...
        if (roadmapError || !roadmap) {
            console.error("Roadmap creation failed:", roadmapError);
            return { success: false, error: roadmapError?.message || "Failed to create project" };
        }
        ...
        if (nodesError || !insertedNodes) {
            ...
            // Even if nodes fail, we return the roadmap ID so user isn't stuck
            return { success: true, id: roadmap.id, warning: "Project created but incomplete modules." };
        }
        ...
        console.error("createProject fatal error:", error);
        return { success: false, error: "An unexpected error occurred while creating your project." };
    }
}
